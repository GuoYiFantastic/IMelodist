...
model_name = "internlm2_chat_7b_qlora_music_e3_v1_yifan"
model_path = os.path.join("./work_dirs", model_name, "hf_merge")
tmp = "./chat/tmp"
...
def gen_wav(text):
    ...
    os.makedirs(f"{tmp}/{launch_time}", exist_ok=True)
    ...
    if abc_notation:
        ...
        tmp_abc = f"{tmp}/{launch_time}/{abc_time}.abc"
        with open(tmp_abc, "w") as abc_file:
            abc_file.write(abc_notation)

        # Convert the temporary ABC file to a MIDI file
        tmp_midi = f"{tmp}/{launch_time}/{abc_time}.mid"
        musicStream = converter.parse(abc_notation, format="abc")
        musicStream.write("midi", fp=tmp_midi)

        # Convert xml to SVG and WAV using MuseScore (requires MuseScore installed)
        svg_file = f"{tmp}/{launch_time}/{abc_time}.svg"
        wav_file = f"{tmp}/{launch_time}/{abc_time}.wav"
